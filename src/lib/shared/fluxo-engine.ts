// Engine de execução de fluxos visuais — Node.js version (migrado de Deno/Supabase Edge Function)
...
async function executarNo(
  ...
  switch (tipo) {
    ...
    case "msg_produto": {
      ...
      if (cat) q = (q as any).ilike("categoria", `%${cat}%`);
      if (gen !== "todos") q = (q as any).in("genero", [gen, "unissex"]);
      if (precoMax > 0) q = (q as any).lte("preco", precoMax);
      if (ordem === "preco_asc") q = (q as any).order("preco", { ascending: true });
      else if (ordem === "preco_desc") q = (q as any).order("preco", { ascending: false });
      else if (ordem === "novidade") q = (q as any).order("criado_em", { ascending: false });
...
function proxAposAguardo(data: FluxoData, nodeId: string, varName: string, ctx: Ctx): string | null {
  ...
  const tipo = node?.data?.tipo;
  if (tipo === "capturar_cep") return nextNodeFrom(data.edges, nodeId, ctx.variaveis.__cep_valido__ === true ? "out" : "invalido");
  if (tipo === "capturar_cpf") return nextNodeFrom(data.edges, nodeId, ctx.variaveis.__cpf_valido__ === true ? "out" : "invalido");
  if (tipo === "capturar_dados") return nextNodeFrom(data.edges, nodeId, ctx.variaveis.__campo_valido__ !== false ? "out" : "invalido");
  if (tipo === "msg_botoes") {
